...
impl<'a, P> ObservableExtractor<'a, DFMatrix, P>
where
    P: Potential<Space = DFMatrix>,
{
    pub fn calculate_s_matrix(
        &mut self,
        l: usize,
        asymptotic: &DynAsymptoticStates,
    ) -> MultiChanSMatrix {
        ...
        let mut j_last = DFMatrix::zeros(size, size);
        let mut j_prev_last = DFMatrix::zeros(size, size);
        let mut n_last = DFMatrix::zeros(size, size);
        let mut n_prev_last = DFMatrix::zeros(size, size);
