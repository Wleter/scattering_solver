...
use crate::{
    ...
    types::FMatrix,
    utility::{asymptotic_bessel_j, asymptotic_bessel_n, bessel_j_ratio, bessel_n_ratio}, asymptotic_states::AsymptoticStates,
};

use super::s_matrix::{HasSMatrix, OneChanSMatrix, MultiChanSMatrix};
...
{
    pub fn calculate_s_matrix(&mut self, l: usize, asymptotic: AsymptoticStates<N>) { // -> impl HasSMatrix {
        let r_last = self.result.r_last;
        let r_prev_last = self.result.r_last - self.result.dr;
        let wave_ratio = self.result.wave_ratio;

        let energy = self.collision_params
            .particles
            .internals
            .get_value("energy");
        let mass = self.collision_params
            .particles
            .red_mass();
        ...
        let k_matrix = -(wave_transf * n_prev_last - n_last).try_inverse().unwrap() * (wave_transf * j_prev_last - j_last);
        ...
        for i in 0..open_channel_count {
            i_full += 1;
            while is_open_channel[i_full] == false { i_full += 1 }
            for j in 0..open_channel_count {
                j_full += 1;
                while is_open_channel[j_full] == false { j_full += 1 }
        ...
        todo!()
        // MultiChanSMatrix::new(s_matrix, momenta, asymptotic.entrance_channel)
    }
}
