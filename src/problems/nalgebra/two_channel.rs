...
impl TwoChannel {
    ...
    fn potential() -> impl Potential<Space = DMatrix<f64>> {
        ...
        let potential = Diagonal::<DMatrix<f64>, _>::from_vec(vec![potential_lj1, potential_lj2]);
        let coupling = MultiCoupling::<DMatrix<f64>, _>::new_neighboring(vec![coupling]);
    ...
    fn mass_scaling() {
        ...
        let s_lengths: Vec<Complex<f64>> = scalings.iter()
            .map(|scaling| {
                ...
                let mut numerov = DMatrixRatioNumerov::new(&potential, &particles, MultiStepRule::default(), boundary.clone());
                numerov.propagate_to(1e3);
